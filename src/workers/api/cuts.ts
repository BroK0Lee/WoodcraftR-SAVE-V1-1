...
export function createRectangularCut(oc: any, cut: RectangularCut, panelThickness: number): TopoDS_Shape {
  if (!oc) throw new Error("OpenCascade not ready");
  const cutDepth = cut.depth || panelThickness;
  const cutBox = new oc.BRepPrimAPI_MakeBox_2(cut.length, cut.width, cutDepth + 2 * EPSILON_CUT).Shape();
  const translation = new oc.gp_Trsf_1();
  const zPosition = panelThickness + EPSILON_CUT - (cutDepth + 2 * EPSILON_CUT);
  translation.SetTranslation_1(new oc.gp_Vec_4(cut.positionX - cut.length / 2, cut.positionY - cut.width / 2, zPosition));
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
export function createCircularCut(oc: any, cut: CircularCut, panelThickness: number): TopoDS_Shape {
  if (!oc) throw new Error("OpenCascade not ready");
  const cutDepth = cut.depth || panelThickness;
  const cylinder = new oc.BRepPrimAPI_MakeCylinder_2(cut.radius, cutDepth + 2 * EPSILON_CUT, 2 * Math.PI).Shape();
  const translation = new oc.gp_Trsf_1();
  const zPosition = panelThickness + EPSILON_CUT - (cutDepth + 2 * EPSILON_CUT);
  translation.SetTranslation_1(new oc.gp_Vec_4(cut.positionX, cut.positionY, zPosition));
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
export async function applyAllCuts(oc: any, panel: TopoDS_Shape, cuts: Cut[], panelThickness: number): Promise<{ resultShape: TopoDS_Shape; cuttingInfo: CuttingInfo; }> {
  if (!oc) throw new Error("OpenCascade not ready");
  ...
  for (const cut of cuts) {
    try {
      ...
      const booleanOp = new oc.BRepAlgoAPI_Cut_3(resultShape, cutShape, new oc.Message_ProgressRange_1());
  ...
  const cuttingInfo: CuttingInfo = { totalCuts: cuts.length, rectangularCuts, circularCuts, totalCutArea, totalCutVolume, failedCuts };
