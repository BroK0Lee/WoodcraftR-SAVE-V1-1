...
export async function createPanelWithCuts(oc: any, config: PanelWithCutsConfig): Promise<PanelWithCutsResult> {
  ...
  if (shape === "circle") {
    ...
    const rawCylinder = new oc.BRepPrimAPI_MakeCylinder_2(radius, dimensions.thickness, 2 * Math.PI).Shape();
    ...
    basePanel = new oc.BRepPrimAPI_MakeBox_2(dimensions.length, dimensions.width, dimensions.thickness).Shape();
  }

  const { resultShape, cuttingInfo } = await applyAllCuts(oc, basePanel, cuts, dimensions.thickness);
