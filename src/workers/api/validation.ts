...
export function validateCutFeasibility(
  panel: PanelSpec,
  cuts: CutSpec[]
): ValidationResult {
  const errors: string[] = [];
  ...
  for (let i = 0; i < cuts.length; i += 1) {
    ...
    if (c.type === "rect") {
      ...
      if (c.width <= 0 || c.height <= 0)
        errors.push(`Rect ${i} dimensions invalides`);
      const hw = c.width / 2;
      const hh = c.height / 2;
      if (
        Math.abs(c.x) + hw > halfPanelW - margin ||
        Math.abs(c.y) + hh > halfPanelH - margin
      ) {
      ...
      if (
        Math.abs(c.x) + c.radius > halfPanelW - margin ||
        Math.abs(c.y) + c.radius > halfPanelH - margin
      ) {
    ...
    if (spacing > 0) {
      for (let j = i + 1; j < cuts.length; j += 1) {
        ...
        if (dist < spacing)
          errors.push(
            `Découpes ${i} et ${j} trop proches (dist=${dist.toFixed(
              2
            )} < ${spacing})`
          );
      }
...
export function validateSingleCut(
  panel: PanelSpec,
  cut: CutSpec
): ValidationResult {
