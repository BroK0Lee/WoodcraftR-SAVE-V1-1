...
export default function AxesHelper({ scale }: AxesHelperProps) {
  ...
  useEffect(() => {
    ...
    const axisX = createCustomAxis(
      new Vector3(0, 0, 0),
      new Vector3(axesLengths.x, 0, 0),
      0xff0000
    ); // Rouge
    const axisY = createCustomAxis(
      new Vector3(0, 0, 0),
      new Vector3(0, axesLengths.y, 0),
      0x00ff00
    ); // Vert
    const axisZ = createCustomAxis(
      new Vector3(0, 0, 0),
      new Vector3(0, 0, axesLengths.z),
      0x0000ff
    ); // Bleu
    ...
    const originOffset =
      Math.min(axesLengths.x, axesLengths.y, axesLengths.z) * 0.05;
