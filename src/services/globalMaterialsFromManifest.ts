import {
  ...
  type GlobalWoodMaterial,
} from "@/store/globalMaterialStore";
import {
...
function toGlobalMaterial(
  entry: MaterialsManifest["materials"][number]
): GlobalWoodMaterial {
  // Heuristique simple de prix: MDF et panneaux < bois massif
  const idLower = entry.id.toLowerCase();
  let price = 35;
  if (
    idLower.includes("mdf") ||
    idLower.includes("agglomere") ||
    idLower.includes("contreplaque")
  ) {
    price = 29;
  } else if (idLower.includes("osb")) {
    price = 25;
  } else if (
    idLower.includes("teck") ||
    idLower.includes("noyer") ||
    idLower.includes("olivier")
  ) {
    price = 59;
  }
  ...
  return {
    ...
    characteristics: {
      generalDescription:
        entry.carousel.fullDescription ||
        entry.carousel.description ||
        entry.carousel.title,
      colorAspect: {
        dominantColor: entry.carousel.color,
        variations: "",
        grain: "",
      },
...
export async function loadAllMaterialsFromManifest(): Promise<
  GlobalWoodMaterial[]
> {
...
export async function initializeMaterialsWithDefault(
  defaultId: string
): Promise<void> {
  const { setLoading, setMaterials, setSelectedMaterialId, setError } =
    useGlobalMaterialStore.getState();
  try {
    setLoading(true);
    ...
    setError(
      e instanceof Error ? e.message : "Erreur de chargement des matériaux"
    );
  } finally {
    setLoading(false);
  }
